...
### 3.2 验证 MCP 注册数量（13 tools + 6 prompts + 10 resources）
...
预期：`tools=17 prompts=6 resources=11`。
...
| `yolo_register_model` | 把指定 job 的 run 产物注册到 MLflow Model Registry（可选直接设为 approved） |
| `yolo_promote_model_version` | 给模型版本设置 alias（默认 `approved`） |
| `yolo_list_registered_models` | 查看注册模型摘要或指定模型的版本列表 |
...
当前可枚举资源为 10 个，另外还支持参数化日志资源：`yolo://jobs/{jobId}/log`。
...
| `yolo://models/registry` | registered-models | MLflow Model Registry 摘要（模型名、aliases、最新版本） |
