...
3. **数据集导出（可选）** → `cvat_export_dataset` 导出并生成可训练的 data.yaml
4. **启动训练** → 调用 `yolo_start_training`（异步，立即返回 jobId + runId）
5. **监控进度** → 周期调用 `yolo_get_status`（传入 jobId + runId）
6. **任务管理** → `yolo_list_jobs` 列出所有任务，`yolo_get_job` 查询单任务详情
7. **停止训练** → `yolo_stop_training`（传入 jobId + runId）
8. **验证评估** → `yolo_validate`（在验证集上运行已完成任务的 best 权重）
9. **导出模型** → `yolo_export`（导出 ONNX/TensorRT/CoreML 等格式）
10. **自动调参** → `yolo_auto_tune`（网格搜索 lr/batch/imgSize，串行执行多个 trial）
...
def cvat_export_dataset(
    ...
    datasetName: Annotated[
        str,
        Field(description="导出后写入远程 datasets 目录的名称（会自动清洗）。"),
    ],
    envId: Annotated[
        str,
        Field(
            default="default",
            description="导出目标环境 ID，用于选择对应 SSH（来自 YOLO_SSH_ENVS）。",
        ),
    ] = "default",
