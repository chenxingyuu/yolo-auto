...
4. **数据集管理（可选）** → `cvat_list_projects` / `cvat_list_tasks` / `cvat_analyze_dataset`
5. **确认导出格式（可选）** → `cvat_list_formats` 查看 CVAT 当前支持的导入导出格式
6. **数据集导出（可选）** → `cvat_export_dataset` / `cvat_export_project_dataset`
   仅 **提交** 云导出，立即返回 `rqId`（后台队列，耗时可很长）
7. **查看导出队列** → `cvat_get_request` / `cvat_list_requests`，看 `status`
   （queued / started / finished / failed）
8. **一键导出并同步（可选）** → `cvat_export_and_sync_*`：提交后 **轮询** `/api/requests`
   至 finished，再 `yolo_sync_dataset`
9. **同步数据集（可选）** → `yolo_sync_dataset`：MinIO 已有 zip 后解压到训练容器
10. **启动训练** → 调用 `yolo_start_training`（异步，立即返回 jobId + runId）
11. **监控进度** → 周期调用 `yolo_get_status`（传入 jobId + runId）
12. **任务管理** → `yolo_list_jobs` 列出所有任务，`yolo_get_job` 查询单任务详情
13. **停止训练** → `yolo_stop_training`（传入 jobId + runId）
14. **验证评估** → `yolo_validate`（在验证集上运行已完成任务的 best 权重）
15. **导出模型** → `yolo_export`（导出 ONNX/TensorRT/CoreML 等格式）
16. **自动调参** → `yolo_auto_tune`（网格搜索 lr/batch/imgSize，串行执行多个 trial）
