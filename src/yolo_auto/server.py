...
5. **一键导出并同步（可选）** → `cvat_export_and_sync_dataset` 导出后立即同步到训练容器
6. **同步数据集（可选）** → `yolo_sync_dataset` 把导出 zip 从 MinIO 解压到训练容器
7. **启动训练** → 调用 `yolo_start_training`（异步，立即返回 jobId + runId）
8. **监控进度** → 周期调用 `yolo_get_status`（传入 jobId + runId）
9. **任务管理** → `yolo_list_jobs` 列出所有任务，`yolo_get_job` 查询单任务详情
10. **停止训练** → `yolo_stop_training`（传入 jobId + runId）
11. **验证评估** → `yolo_validate`（在验证集上运行已完成任务的 best 权重）
12. **导出模型** → `yolo_export`（导出 ONNX/TensorRT/CoreML 等格式）
13. **自动调参** → `yolo_auto_tune`（网格搜索 lr/batch/imgSize，串行执行多个 trial）
...
def cvat_export_and_sync_dataset(
    ...
    statusCheckPeriod: Annotated[
        int | None,
        Field(default=None, description="可选：CVAT 导出队列轮询间隔（秒）。"),
    ] = None,
    includeImages: Annotated[
        bool,
    ...
    """导出 CVAT 数据集到云存储后，同步到训练容器并返回可训练 dataConfigPath。"""
