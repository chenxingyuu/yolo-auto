...
3. **数据集管理（可选）** → `cvat_list_projects` / `cvat_list_tasks` / `cvat_analyze_dataset`
4. **确认导出格式（可选）** → `cvat_list_formats` 查看 CVAT 当前支持的导入导出格式
5. **数据集导出（可选）** → `cvat_export_dataset` / `cvat_export_project_dataset`
   仅 **提交** 云导出，立即返回 `rqId`（后台队列，耗时可很长）
6. **查看导出队列** → `cvat_get_request` / `cvat_list_requests`，看 `status`
   （queued / started / finished / failed）
7. **一键导出并同步（可选）** → `cvat_export_and_sync_*`：提交后 **轮询** `/api/requests`
   至 finished，再 `yolo_sync_dataset`
8. **同步数据集（可选）** → `yolo_sync_dataset`：MinIO 已有 zip 后解压到训练容器
9. **启动训练** → 调用 `yolo_start_training`（异步，立即返回 jobId + runId）
10. **监控进度** → 周期调用 `yolo_get_status`（传入 jobId + runId）
11. **任务管理** → `yolo_list_jobs` 列出所有任务，`yolo_get_job` 查询单任务详情
12. **停止训练** → `yolo_stop_training`（传入 jobId + runId）
13. **验证评估** → `yolo_validate`（在验证集上运行已完成任务的 best 权重）
14. **导出模型** → `yolo_export`（导出 ONNX/TensorRT/CoreML 等格式）
15. **自动调参** → `yolo_auto_tune`（网格搜索 lr/batch/imgSize，串行执行多个 trial）

## 关键约定
- 远程路径：数据集在 `/workspace/datasets/`，模型权重在 `/workspace/models/`
- 启动训练前务必依次调用 `yolo_setup_env` 与 `yolo_check_dataset`
...
def yolo_setup_env(
    ...
    """
    return setup_env(SSH, SETTINGS.yolo_work_dir, dataConfigPath, model)
...
@mcp.tool(name="cvat_get_request")
def cvat_get_request(
    rqId: Annotated[
        str,
        Field(description="CVAT 后台请求 ID（云导出接口返回的 rqId，对应 /api/requests/{id}）。"),
    ],
) -> dict[str, Any]:
    """查询 CVAT 队列中单条请求状态（导出进度等）。"""
    return _with_cvat_client(lambda client: ok({"request": client.get_request(rqId)}))


@mcp.tool(name="cvat_list_requests")
def cvat_list_requests(
    projectId: Annotated[
        int | None,
        Field(default=None, description="按 project_id 过滤（可选）。"),
    ] = None,
    taskId: Annotated[
        int | None,
        Field(default=None, description="按 task_id 过滤（可选）。"),
    ] = None,
    status: Annotated[
        str | None,
        Field(
            default=None,
            description="按状态过滤：queued / started / finished / failed（可选）。",
        ),
    ] = None,
    page: Annotated[int | None, Field(default=None, description="分页页码（可选）。")] = None,
    pageSize: Annotated[
        int,
        Field(default=20, ge=1, le=500, description="每页条数。"),
    ] = 20,
) -> dict[str, Any]:
    """分页列出 CVAT /api/requests（可用 projectId、taskId、status 过滤）。"""
    return _with_cvat_client(
        lambda client: list_cvat_request_queue(
            client,
...
def cvat_export_dataset(
    ...
    statusCheckPeriod: Annotated[
        ...
        Field(
            default=None,
            description="已弃用。云导出改为异步提交，请用返回的 rqId + cvat_get_request 轮询。",
        ),
    ] = None,
    ...
    """提交 CVAT 数据集云导出（立即返回 rqId，不等待队列跑完）。"""
    return _with_cvat_client(
...
@mcp.tool(name="cvat_export_and_sync_dataset")
def cvat_export_and_sync_dataset(
    taskId: Annotated[int, Field(gt=0, description="CVAT 任务 ID。")],
    datasetName: Annotated[
        str,
        Field(description="导出文件名和本地目录中使用的数据集名称（会自动清洗）。"),
    ],
    formatName: Annotated[
        str,
        Field(
    ...
    statusCheckPeriod: Annotated[
        ...
        Field(
            default=None,
            description=(
                "轮询间隔（秒）：未指定 pollSeconds 时用此项或 CVAT_EXPORT_POLL_SECONDS。"
            ),
        ),
    ] = None,
    pollSeconds: Annotated[
        float | None,
        Field(
            default=None,
            description="轮询 /api/requests 间隔（秒）；默认取 CVAT_EXPORT_POLL_SECONDS。",
        ),
    ] = None,
    maxWaitSeconds: Annotated[
        float | None,
        Field(
            default=None,
            description="等待导出 finished 的最长时间（秒）；默认 CVAT_EXPORT_MAX_WAIT_SECONDS。",
        ),
    ] = None,
    includeImages: Annotated[
        bool,
        Field(default=True, description="是否在导出包中包含原图，默认 true。"),
    ] = True,
    envId: Annotated[
        str,
        Field(
    ...
    """提交云导出后轮询队列至 finished，再同步到训练容器并返回 dataConfigPath。"""
    ssh_client = SSH_BY_ENV.get(envId, SSH)
    effective_cloud_storage_id = (
        cloudStorageId if cloudStorageId is not None else SETTINGS.cvat_cloud_storage_id
    )
    eff_poll = float(
        pollSeconds
        if pollSeconds is not None
        else (
            statusCheckPeriod
            if statusCheckPeriod is not None
            else SETTINGS.cvat_export_poll_seconds
        )
    )
    eff_max = float(
        maxWaitSeconds
        if maxWaitSeconds is not None
        else SETTINGS.cvat_export_max_wait_seconds
    )
    return _with_cvat_client(
        lambda client: export_and_sync_cvat_dataset(
            client,
            ssh_client,
            task_id=taskId,
            dataset_name=datasetName,
            format_name=formatName,
            include_images=includeImages,
            cloud_storage_id=effective_cloud_storage_id,
            cloud_filename=cloudFilename,
            status_check_period=statusCheckPeriod,
...
@mcp.tool(name="cvat_export_project_dataset")
def cvat_export_project_dataset(
    projectId: Annotated[int, Field(gt=0, description="CVAT 项目 ID。")],
    datasetName: Annotated[
        str,
        Field(description="导出文件名中使用的数据集名称（会自动清洗）。"),
    ],
    ...
    cloudFilename: Annotated[
        ...
        Field(
            default=None,
            description="可选：导出到 CVAT 云存储的文件名（如 exports/my-project.zip）。",
        ),
    ] = None,
    statusCheckPeriod: Annotated[
        int | None,
        Field(
            default=None,
            description="已弃用。云导出异步；请用 rqId + cvat_get_request / cvat_list_requests。",
        ),
    ] = None,
    includeImages: Annotated[
        bool,
    ...
    """提交整项目云导出（立即返回 rqId，不等待队列）。"""
    return _with_cvat_client(
        lambda client: export_cvat_project_dataset(
            client,
            project_id=projectId,
            dataset_name=datasetName,
            format_name=formatName,
...
@mcp.tool(name="cvat_export_and_sync_project_dataset")
def cvat_export_and_sync_project_dataset(
    projectId: Annotated[int, Field(gt=0, description="CVAT 项目 ID。")],
    datasetName: Annotated[
        str,
        Field(description="导出文件名和本地目录中使用的数据集名称（会自动清洗）。"),
    ...
    statusCheckPeriod: Annotated[
        ...
        Field(
            default=None,
            description="轮询间隔（秒）：未指定 pollSeconds 时作为 poll 间隔。",
        ),
    ] = None,
    pollSeconds: Annotated[
        float | None,
        Field(
            default=None,
            description="轮询 /api/requests 间隔（秒）；默认 CVAT_EXPORT_POLL_SECONDS。",
        ),
    ] = None,
    maxWaitSeconds: Annotated[
        float | None,
        Field(
            default=None,
            description="等待 finished 的最长时间（秒）；默认 CVAT_EXPORT_MAX_WAIT_SECONDS。",
        ),
    ] = None,
    includeImages: Annotated[
        bool,
    ...
    return _with_cvat_client(
        lambda client: export_and_sync_cvat_project_dataset(
            client,
            ssh_client,
            project_id=projectId,
            dataset_name=datasetName,
